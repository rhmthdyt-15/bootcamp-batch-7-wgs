...
const formFields = [
    ...
    { name: 'telepon', label: 'Telepon', type: 'text' } // Ganti type menjadi 'text'
]
...
function EditSupplier() {
    ...
    useEffect(() => {
        axiosJWT
            .get(`http://localhost:5000/suppliers/${id}`, Config)
            .then((response) => {
    ...
    const handleSubmit = (e) => {
        ...
        axiosJWT
            .put(`http://localhost:5000/suppliers/${id}`, formData, Config)
            .then(() => {
                // Navigasi ke halaman supplier setelah berhasil memperbarui
                navigate('/suppliers')

                // Menampilkan pesan sukses
                showSuccessAlert('Supplier Berhasil Diperbarui!')
    ...
    // Render tampilan komponen EditSupplier
    return (
        <div className="bg-white px-4 pt-3 pb-4 rounded-sm border border-gray-200 flex-1">
            <div className="flex justify-between items-center mb-3">
                {/* Judul halaman */}
                <strong className="text-gray-700 font-medium">Edit Supplier</strong>
            </div>
...
// Mengekspor komponen EditSupplier sebagai default
